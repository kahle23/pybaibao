...
# 本库顶级包名：用于自身识别（如 python -m baibao 场景），避免在多处硬编码 "baibao"
...
def get_own_top_package_name() -> str:
    ...
    用于 ``python -m baibao`` 等场景下识别自身包名，供版本查询、日志标记、
    配置目录等模块统一引用，避免在代码中硬编码 ``"baibao"``。
...
def get_caller_top_package_name() -> str:
    """
    获取调用方的顶级包名（跳过 baibao 内部调用）。

    典型场景：外部模块 A 引入 baibao 后，在 baibao 内部识别出 A 的包名；
    也兼容 ``python -m A`` 启动方式。遍历调用栈找到第一个顶级包名不是
    baibao 的帧；若整条调用栈都在 baibao 内（如 baibao 自身的脚本/测试），
    则回退到本库包名。

    边界说明：REPL、``python -c``、``__main__`` 等场景下 ``__package__``
    可能为空，会被跳过继续向上查找，找不到外部调用方时回退到本库包名。

    Returns:
        调用方的顶级包名；找不到外部调用方时返回本库包名。
    """
    ...
    while frame is not None:
        ...
        if pkg:
            top = pkg.split('.')[0]
            # 严格按顶级包名比较，避免 baibao_ext / baibao2 等被前缀误判为内部调用

...
class DbUtil:
    ...
    提供统一的数据库操作接口。
    所有方法均为静态方法，通过数据库配置名获取对应的 DbPool 实例进行操作。
    """
    ...
    def get_driver(db_type: str):
        ...
        """
        driver_tuple = DbUtil.DRIVER_MAP.get(db_type.lower())
        if not driver_tuple:
    ...
    def get_pool(cfg_name: Optional[str] = None):
        ...
        """
        if cfg_name is None:
            cfg_name = DbUtil.DEFAULT_CFG_NAME

        if cfg_name not in DbUtil._pools:
            if cfg_name == DbUtil.DEFAULT_CFG_NAME:
                try:
                    cfg = DbCfg.load_json_config("./db.config")
                    DbUtil.set_pool(cfg_name, cfg)
                except Exception as e:
                    raise KeyError(f"自动加载默认配置失败: {e}")
            else:
                raise KeyError(f"未找到配置名 '{cfg_name}' 对应的 DbPool，请先调用 set_pool() 设置")

        return DbUtil._pools[cfg_name]
    ...
    def set_pool(cfg_name: str, db_pool: Union[DbPool, DbCfg]) -> None:
        ...
        Args:
            cfg_name: 数据库配置名
            dbPool: DbPool 实例或 DbCfg 配置对象
        """
        if isinstance(db_pool, DbPool):
            DbUtil._pools[cfg_name] = db_pool
        elif isinstance(db_pool, DbCfg):
            DbUtil._pools[cfg_name] = DbPool(db_pool)
        else:
    ...
    def exec(sql: str, params: Optional[Tuple[Any, ...]] = None, cfg_name: Optional[str] = None):
        ...
        """
        connection = DbUtil.get_connection(cfg_name)
        try:
                ...
                result = [
                    {
                        ...
                        k: float(v)
                        # 条件判断
                        if (to_float and isinstance(v, Decimal))
                        # 否则保持原值 v
                        else v
                        # 遍历字典的每个键值对

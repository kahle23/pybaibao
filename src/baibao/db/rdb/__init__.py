...
典型用法::
    ...
    from baibao.db.rdb import rdb, MysqlClient, RdbCfg

    # 驱动：为不同环境注册同类型的 MySQL 实例（实现类相同、连接配置不同）
    rdb.register("dev",  MysqlClient(RdbCfg(db_type='mysql', host='dev-host', ...)))
    rdb.register("test", MysqlClient(RdbCfg(db_type='mysql', host='test-host', ...)))

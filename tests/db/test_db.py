...
from baibao.db.db_pool import DbPool, DbCfg
...
class TestGetPool(unittest.TestCase):
    ...
    def test_get_pool_with_custom_name(self, mock_cfg):
        ...
        db.set_pool('test_cfg', test_pool)

        pool = db.get_pool('test_cfg')
    ...
    def test_get_pool_unknown_name_raises(self):
        ...
        with self.assertRaises(KeyError) as context:
            db.get_pool('nonexistent')

        self.assertIn("未找到配置名", str(context.exception))


class TestSetPool(unittest.TestCase):
    """测试 set_pool 方法"""
    ...
    def test_set_pool_with_dbpool_instance(self):
        """测试设置 DbPool 实例"""
        test_pool = MagicMock(spec=DbPool)
        db.set_pool('test', test_pool)

        self.assertEqual(db._pools['test'], test_pool)
    ...
    def test_set_pool_with_dbcfg_object(self):
        ...
        with patch('baibao.db.db_pool.DbPool.__init__', return_value=None):
            ...
            db.set_pool('test', test_cfg)

        self.assertIsInstance(db._pools['test'], DbPool)

    def test_set_pool_invalid_type_raises(self):
        """测试设置无效类型时抛出 TypeError"""
        with self.assertRaises(TypeError) as context:
            db.set_pool('test', "invalid")

        self.assertIn("必须是 DbPool 或 DbCfg 类型", str(context.exception))


class TestRemovePool(unittest.TestCase):
    """测试 remove_pool 方法"""
    ...
    def test_remove_default_pool(self):
        """测试移除默认配置的池"""
        test_pool = MagicMock(spec=DbPool)
        db.set_pool('', test_pool)

        db.remove_pool()

        self.assertNotIn('', db._pools)

    def test_remove_custom_pool(self):
        """测试移除自定义配置的池"""
        test_pool = MagicMock(spec=DbPool)
        db.set_pool('my_cfg', test_pool)

        db.remove_pool('my_cfg')

        self.assertNotIn('my_cfg', db._pools)

    def test_remove_nonexistent_pool(self):
        """测试移除不存在的池不会报错"""
        db.remove_pool('nonexistent')  # 不应抛出异常
...
class TestExec(unittest.TestCase):
    ...
    @patch('baibao.db.get_connection')
    def test_execute_success(self, mock_get_conn):
        ...
        db.set_pool('', mock_pool)

        result = db.exec("INSERT INTO test VALUES(1)")
...
class TestClear(unittest.TestCase):
    ...
    def test_clear_removes_all_pools(self):
        ...
        db.set_pool('cfg1', test_pool1)
        db.set_pool('cfg2', test_pool2)

        db.clear()

        self.assertEqual(len(db._pools), 0)
    ...
    def test_clear_after_adding_pools(self):
        ...
        db.set_pool('new_cfg', new_pool)

        self.assertIn('new_cfg', db._pools)
        self.assertEqual(db._pools['new_cfg'], new_pool)

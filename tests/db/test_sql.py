...
class TestExec(unittest.TestCase):
    """测试 exec 方法 - 执行 SQL 语句（INSERT、UPDATE、DELETE）"""

    def setUp(self):
        sql.clear()

    @patch('baibao.db.get_connection')
    def test_exec_success(self, mock_get_conn):
        """测试执行成功并返回影响行数"""
        mock_conn = MagicMock()
        ...
        result = sql.exec("INSERT INTO test VALUES(1)")
    ...
    @patch('baibao.db.get_connection')
    def test_exec_rollback_on_error(self, mock_get_conn):
        """测试出错时回滚"""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
